...
func PayOpCosts(sc datatype.ServiceContainer) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token, err := sc.TokenService.InsertToken(
			...
			body.Purpose,
			"0.0", // hard cap
			add.Hex(), //address of the deployed token
			tx.Hash().Hex(), //address of the transaction that was used to deploy the token
		)
		if err != nil {
			c.String(http.StatusBadRequest, err.Error())
		}
